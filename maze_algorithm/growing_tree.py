...
from maze_algorithm.maze import MazeGenerator
...
class GrowTree(MazeGenerator):

    def __init__(self, width: int, height: int,
                 entry: tuple[int], finish: tuple[int],
                 perfect: bool, seed: str) -> None:
        super().__init__(width, height, entry, finish, perfect, seed)

    def look_neighbor(self, x1, y1) -> list:
        ...
        for x, y, c in zip(x_axes, y_axes, compass):
            if (x1+x >= 0 and y1+y >= 0
                ...
                and self.grid[y1+y][x1+x] == 15
                    and self.grid[y1+y][x1+x] != -1):
