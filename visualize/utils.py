...
def check_change(config: 'MazeConfig', entry: str, exit_coord:
    ...
    """
    if config.ENTRY != entry and config.EXIT != exit_coord and show_path:
        return ("\n" + "The entrance and exit cannot be on the 42 walls."
                " They have been moved to the entrance and exit of the maze."
                + "\n")

    if config.ENTRY != entry and show_path:
        return ("\n" + "The entrance cannot be on the walls 42. "
                "It has been moved to the start of the maze." + "\n")

    if config.EXIT != exit_coord and show_path:
